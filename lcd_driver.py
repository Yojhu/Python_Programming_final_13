...
class LCD:
    ...
    def r(self, string, line=2):
        ...
        shif = string + " " * self.LCD_WIDTH
    
        for i in range(len(shif) - self.LCD_WIDTH + 1):
            display = shif[i:i + self.LCD_WIDTH] 
            self.lcd_byte(lcd_line, self.LCD_CMD)
            for j in display:
                self.lcd_byte(ord(j), self.LCD_CHR)
            time.sleep(0.3)
    def clear(self):
        # clear LCD display
